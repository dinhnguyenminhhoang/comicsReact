...
const handleCheckUserFolowed = (userId, comicId) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (data) {
                resolve({
                    message: "user folowed",
                    isCheck: true,
                    errCode: 0,
                });
            } else {
                resolve({
                    message: "user not folow",
                    isCheck: false,
                    errCode: 0,
                });
            }
                    ...
                    .then((comic) => {
                        if (comic) {
                            db.Category.findAll({
                                ...
                                const categories_comic = categories.map(
                                    (category) => ({
                                        comicId: comic.id,
                                ...
                                db.Comic_Categories.bulkCreate(categories_comic)
                                    .then(() => {
                                        resolve({
                                            errCode: 0,
                                            message:
                                                "Added a category for success stories",
                                        });
                                    })
                                    .catch((error) => {
                                        resolve({
                                            error: 1,
                                            message:
                                                "Added a category for failed stories",
                                            error: error,
                                        });
                                    });
                            });
